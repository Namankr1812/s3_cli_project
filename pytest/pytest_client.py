...
from pytest_dev import pytest_multi_part_upload
from pytest_dev import pytest_multi_part_delete
from pytest_dev import pytest_multi_part_download
from pytest_dev import pytest_multi_part_list
from pytest_dev import pytest_multi_part_logs
from pytest_dev import pytest_multi_part_new_folder_creation
...
def user():
    ...
    while True:
        ...
        if choice == "1":
            ...
            pytest_multi_part_new_folder_creation.create_folder(folder_name, bucket)
            logging.info(f"Created folder: {folder_name}")
        elif choice == "2":
            folder_path = click.prompt("Enter the folder path")
            pytest_multi_part_upload.upload_folder(folder_path, bucket)
            logging.info(f"Uploaded files from {folder_path} to S3 bucket {bucket}")
        elif choice == "3":
            pytest_multi_part_list.list_folder_contents(bucket)
            ...
            pytest_multi_part_delete.delete_files_interactive(bucket, files_to_delete)
            logging.info(f"Deleted files {files_to_delete} from S3 bucket {bucket}")
        elif choice == "5":
            pytest_multi_part_download.download_from_s3(bucket)
            logging.info(f"Downloaded a file from S3 bucket {bucket}")
        elif choice == "6":
            pytest_multi_part_logs.view_s3_bucket_logs(bucket)
        elif choice == "7":
